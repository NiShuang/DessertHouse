...
<div class="frame">
	<span class="title"><%=request.getAttribute("name")%>的订单</span>
	<br></br>
	<%if(request.getAttribute("point")!=null){
		int point = (int)request.getAttribute("point");
	%>
		<span style= 'color: black'>&nbsp;&nbsp;&nbsp;&nbsp;<%=request.getAttribute("name")%>获得 <%=point %> 积分</span>
	<%}%>
	<a class="add" href="http://localhost:8080/DessertHouseWeb/clerk/queryReserve.jsp">返回</a>
	<br></br>
    ...
    <tr>
        <td><%=index%></td>    
        <td><jsp:getProperty name="item" property="member"/></td>  
        <td><%=store %></td>    
        <td><%=commodity %></td>
        ...
        <td><jsp:getProperty name="item" property="date"/></td>  
         <td><a class="order" href="http://localhost:8080/DessertHouseWeb/SellReserveServlet?id=<jsp:getProperty name="item" property="id"/>&member=<jsp:getProperty name="item" property="member"/>" onclick="conf()">取货</a> </td>  
    </tr>    
    <%} %>    
