...
<body>
		...
		String isBefore = String.valueOf(request.getAttribute("isBefore"));
		%>
<div class="frame">
	<span class="title">产品计划</span>
	<br></br>
	<a class="approve" href="http://localhost:8080/DessertHouseWeb/ApproveServlet?store=<%=store_id%>&date=<%=date%>">一键批准</a>
	<br></br>
<span class="word">店面</span><div class="tab">
  <ul class="tab_menu">
   <%
	...
	%>
	<li class="tab_menu-item" <%if(storeList.getStoreList(i).getId()==store_id){ %>style="background-color: #9b59b6"<%} %>><a href="http://localhost:8080/DessertHouseWeb/ShowPlanSubmitedServlet?store=<jsp:getProperty name="item1" property="id"/>&date=<%=date%>"><jsp:getProperty name="item1" property="name"/></a></li>
    <%} %> 
  </ul>
</div>
<br></br>
  <form name="date" method="post" action="http://localhost:8080/DessertHouseWeb/ShowPlanSubmitedServlet?store=<%=store_id%>" >
<div class="demo6">
...
<table class="bordered">
    ...
    <tr>
        ...
        <td><%=state %></td>
        <%if(isBefore=="false"){if(myState==3){%>
        <td><a class="order" href="http://localhost:8080/DessertHouseWeb/ApproveOneServlet?id=<jsp:getProperty name="item" property="id"/>&result=2&store=<%=store_id%>&date=<%=date%>">批准</a></td>
    	<%} else if(myState==2){%>        <td><a class="order" href="http://localhost:8080/DessertHouseWeb/ApproveOneServlet?id=<jsp:getProperty name="item" property="id"/>&result=3&store=<%=store_id%>&date=<%=date%>">不批</a></td>
    	<%} else{ %>        <td><a class="order" href="http://localhost:8080/DessertHouseWeb/ApproveOneServlet?id=<jsp:getProperty name="item" property="id"/>&result=2&store=<%=store_id%>&date=<%=date%>">批准</a> <a class="order" href="http://localhost:8080/DessertHouseWeb/ApproveOneServlet?id=<jsp:getProperty name="item" property="id"/>&result=3&store=<%=store_id%>&date=<%=date%>">不批</a></td>
