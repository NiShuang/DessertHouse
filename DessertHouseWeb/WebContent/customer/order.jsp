...
<body>
		...
		int state = Integer.parseInt(String.valueOf(request.getAttribute("state")));%>
<div class="frame">
<div class="row">
...
<ul class="tab_menu">
   <%
	...
	%>
	<li class="tab_menu-item" <%if(storeList.getStoreList(i).getId()==store_id){ %>style="background-color: #9b59b6"<%} %>><a href="http://localhost:8080/DessertHouseWeb/ShowMenuServlet?store=<jsp:getProperty name="item1" property="id"/>&date=<%=date%>"><jsp:getProperty name="item1" property="name"/></a></li>
    <%} %> 
  </ul>
</div>
</div>
...
<br></br>
  <form name="date" method="post" action="http://localhost:8080/DessertHouseWeb/ShowMenuServlet?store=<%=store_id%>" >
<div class="demo6">
...
<table class="bordered">
    ...
    <tr>
        ...
        <td><%=sale %></td>
        <%if(state==1){%>
        <td><a class="order" href="http://localhost:8080/DessertHouseWeb/ShowOneReserveServlet?id=<jsp:getProperty name="item" property="id"/>" >预定</a> </td>
   		<%} else{%>        <td><a class="order" href="javascript:void(0);" onClick="func()">预定</a> </td>
